...
메시지 16개짜리 시퀀스라 두 그림으로 나눴습니다 — 아래 그림이 위 그림에 이어집니다.

![요청 시퀀스 1/2 — 지식 베이스 경로](diagrams/sequence.svg)

사용자가 키와 질문을 넣고 "Get Answer"를 누르면 `query_assistant`가 `agent.run(question)`을 호출합니다. 이 그림은 오늘 자 agno로는 임포트조차 안 되는 부분(Step 1·3·4·5)을 걷어내고, 소스가 **의도한** 설계를 그린 것입니다 — OpenAI에 도구 스키마를 담아 1차 요청을 보내면 모델이 지식 베이스 검색 도구(`search_knowledge_base`)를 먼저 부르기로 판단하고, 그 실행에는 질의를 벡터로 바꾸는 임베딩 호출과 pgvector 검색이 함께 끼어 있습니다.

지식 베이스 결과를 받은 다음부터는 아래 그림으로 이어집니다.

![요청 시퀀스 2/2 — 웹 검색 분기와 최종 응답](diagrams/extra-followup.svg)

지식 베이스 결과를 담아 2차 요청을 보내면, 모델이 거기서 못 찾았다고 판단할 때만 DuckDuckGo(`web_search`)를 추가로 부르고 그 결과로 3차 요청을 보냅니다. 다만 Step 6에서 확인했듯 이 순서를 강제하는 코드는 없으므로, 실행할 때마다 모델이 지식 베이스만 보고 끝낼 수도, 웹만 볼 수도, 둘 다 건너뛸 수도 있습니다 — 이 그림의 웹 검색 구간은 매 실행마다 달라질 수 있는 모델의 선택이지 고정된 경로가 아닙니다. 마지막 화살표는 실제로 벌어지는 일입니다: `agent.run()`이 돌려주는 `RunOutput`을 `query_assistant`가 반복하려다 `TypeError`로 멈추는 것으로, Step 7에서 유효하지 않은 키로 직접 확인했습니다. 이 두 그림은 처음부터 끝까지 유효한 키로 한 번에 이어지는 것을 보지는 못했고, DB에 접속하는 구간을 포함해 각 구간을 개별적으로 확인한 것을 순서대로 이어붙인 것입니다.
