...
- `kb.add_content(...)`가 나오는 세 곳(`rag_tutorials/agentic_rag_embedding_gemma/agentic_rag_embeddinggemma.py:43`, `rag_tutorials/agentic_rag_embedding_gemma/agentic_rag_embeddinggemma.py:49`, `rag_tutorials/agentic_rag_embedding_gemma/agentic_rag_embeddinggemma.py:85`)를 모두 `insert(...)`로 바꿔 실행해보기. URL을 넣기 전, 커밋된 로컬 PDF 자동 적재만으로 **행 개수가 11 → 22로 늘어납니다**(직접 확인 — 아래). 원인은 `_build_content_hash`(agno 3.0.10, `agno/knowledge/knowledge.py`)가 이름·설명·메타데이터 없이 `path` 문자열 그대로로 해시를 만드는데, `LOCAL_PDF_PATH`(`rag_tutorials/agentic_rag_embedding_gemma/agentic_rag_embeddinggemma.py:10`)가 `os.path.dirname(__file__)` 기준이고 `streamlit run`은 `main_script_path = os.path.abspath(file)`로 `__file__`을 실행한 컴퓨터의 절대경로로 항상 바꿔 버리기 때문입니다(소스로 확인, `streamlit/web/cli.py`) — 커밋된 테이블의 행이 가진 해시(원래 경로 문자열이 만든 값)와 독자 컴퓨터의 절대경로가 만드는 해시가 달라서 `insert(upsert=True)`가 "다른 콘텐츠"로 보고 11개를 더 넣습니다. 직접 확인: 커밋된 테이블을 그대로 복사한 스크래치 사본에서 실제 `streamlit run`과 같은 방식으로 `__file__`을 절대경로로 만들어 `insert`를 호출하면 `rows BEFORE: 11` → `rows AFTER: 22`(로그에 "Deleted" 줄 없이 11개가 그대로 추가됨), 같은 절대경로로 다시 실행하면 이번엔 해시가 자기 자신과 일치해 `Deleted 11 records`가 찍히고 22 그대로 유지됩니다. `tmp/lancedb/recipes.lance/`에는 이 과정에서 새 트랜잭션·매니페스트 파일도 여러 개 생깁니다(커밋하지 않도록 주의). 사이드바에서 **새 PDF URL**을 추가하면 더 늘어납니다 — `recipes` 테이블이 몇 행이 되는지 관찰해보기
