...
streamlit 1.51.0의 `streamlit/web/bootstrap.py` 87-90행 (직접 확인. 설치된 패키지 내부라 저장소 경로가 아니며, 다른 버전에서는 줄 번호가 달라질 수 있다)
