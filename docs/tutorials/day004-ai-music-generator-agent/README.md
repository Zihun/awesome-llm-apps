...
| ModelsLab API 키 | 실제 음악을 만드는 ModelsLab 음악 생성 API 호출 인증. 역시 사이드바 입력창에 직접 붙여넣는다 | https://modelslab.com/dashboard/api-keys 가입 후 발급 |
...
`prompt`는 기본값("Generate a 30 second classical music piece")이 이미 채워진 채로 키와 무관하게 항상 표시됩니다. 반면 에이전트 생성부터 버튼, 결과 표시까지는 전부 `if openai_api_key and models_lab_api_key:` 블록 안에 있고, 빠지면 `else`의 사이드바 경고만 뜹니다. Day 3는 버튼에 `disabled=`를 걸어 항상 그려두고 막았지만, 이 앱은 조건을 만족하기 전에는 버튼을 아예 그리지 않습니다 — 더 단순하지만, Step 3의 문제도 이 블록에 함께 있어 키를 채우는 순간 같이 실행됩니다.
...
`OpenAIChat(id="gpt-4o", api_key=openai_api_key)`는 Day 3의 환경변수 방식과 달리 사이드바에서 받은 키를 생성자 인자로 바로 넘깁니다. 이 객체 하나만 놓고 보면 아무 키로나 문제없이 만들어진다는 것은 Day 1·3에서 이미 확인했으므로 여기서는 다시 보이지 않습니다. 문제는 그다음 줄입니다 — `agent_id="ml_music_agent"`는 agno 3.0.9의 `Agent.__init__()`에 더 이상 없는 인자 이름입니다(직접 확인: `inspect.signature(Agent.__init__)`에 `id`는 있고 `agent_id`는 없음).
...
키가 없으면 네트워크 요청 없이 이 문구만 돌아오지만, 유효하지 않은 키는 실제로 ModelsLab 서버까지 요청이 나갑니다.
...
이 버튼과 그 아래 코드 전부가 Step 3에서 이미 멈추는 지점과 같은 블록 안에 있어, 버튼 자체가 그려지지 않는다는 것은 앞서 확인한 그대로입니다. 다만 `try/except`(47번째 줄부터)는 `agent.run(prompt)`부터만 감싸고 있어서, `Agent(...)` 생성 자체의 실패는 이 `try`로 잡히지 않고 Streamlit의 미처리 예외 화면으로 그대로 튀어나옵니다.
...
Day 1·3와 같은 패턴대로 `status=RunStatus.error`, `music.audio`는 `None`입니다. 유효한 키가 있으면 대신 `status=RunStatus.completed`와 함께 `RunOutput.audio`에 오디오 URL이 담겨 돌아옵니다.
...
이 앱은 Day 3의 ElevenLabs 단계와 달리 오디오를 메모리에서 바로 위젯에 넘기지 않고, `music.audio[0].url`로 다시 `requests.get()`을 보내 받은 바이트를 `audio_generations/music_<uuid>.mp3`로 디스크에 저장한 다음(직접 확인: `uuid4`가 이 파일에서 유일하게 실제로 쓰이는 곳입니다 — Day 3의 죽은 import와 대비됩니다) 다시 읽어 재생 위젯과 다운로드 버튼에 넘깁니다. 저장 전 `response.ok`와 `Content-Type` 검증은 Day 3보다 한 단계 더 방어적입니다. `music.audio`가 비어 있으면 "No audio generated"만 뜨고, 위에서 나는 예외는 `except Exception`이 화면과 로그에 함께 남깁니다.
...
사용자가 프롬프트와 키 2종을 입력하고 "Generate Music"을 누르면 — 다만 Step 3에서 본 것처럼 실제 리포 코드는 이 지점에 도달하기 전에 멈춥니다. 아래는 그 한 인자만 고쳤다고 가정했을 때 설계된 흐름입니다. `agent.run(prompt)`가 호출되면 에이전트는 지시문과 `generate_media` 스키마를 프롬프트에 얹어 GPT-4o에 보냅니다. Day 1과 같은 도구 호출 루프입니다 — GPT-4o는 지시문이 코치한 대로 장르·악기·템포·구조를 채운 상세 프롬프트와 함께 `generate_media` tool_call을 돌려주고, 에이전트는 이를 받아 `ModelsLabTools.generate_media()`로 ModelsLab에 실제 생성 요청을 보냅니다. ModelsLab이 오디오 URL을 돌려주면 GPT-4o가 이를 근거로 최종 응답을 `RunOutput`에 담아 돌려줍니다. 여기서부터는 Day 3와 같은 구조로, 다운로드는 에이전트가 아니라 앱 코드가 직접 처리합니다 — `RunOutput.audio[0].url`로 다시 `requests.get()`을 보내 mp3를 받고, 검증한 뒤 `audio_generations/`에 저장하고 같은 바이트를 재생·다운로드 버튼에 넘깁니다. 이 tool_call 왕복은 Day 1·3에서 확인한 agno의 표준 동작에 근거한 설명이며, 이 앱 자체로 끝까지 재현한 사실은 아닙니다 — 직접 확인한 것은 `ModelsLabTools` 단독 호출(Step 4)과 인증 실패 시 `RunOutput`의 모양(Step 6)까지입니다.
...
| 유효하지 않은 ModelsLab 키로 `generate_media()`를 호출하면 `Error: Invalid API Key. Get API key from: https://modelslab.com/dashboard/api-keys` | ModelsLab 서버가 실제로 키를 검사해 에러 JSON을 돌려주고, 코드가 이를 `Error: {message}` 형태로 감싼다(직접 확인) | 유효한 ModelsLab 키로 교체 |
...
- `file_type=FileType.MP3`(`starter_ai_agents/ai_music_generator_agent/music_generator_agent.py:27`)를 `FileType.WAV`로 바꿔 다른 ModelsLab 엔드포인트를 호출해보기 — 단, 이후 저장 로직은 mp3 확장자를 가정하므로 함께 손봐야 함
