...
원인을 PyPI 파일 목록에서 직접 확인했습니다: `chroma-hnswlib` 0.7.6 정식 릴리스는 Windows용 사전 빌드 wheel을 `cp37`부터 `cp311`까지만 올려두었고 `cp312`·`cp313`은 하나도 없습니다(바로 앞 선판 `0.7.6a1`~`a9`에는 Windows용 `cp312` wheel이 있었지만 정식 `0.7.6`에서는 그것만 빠졌습니다 — macOS·Linux용 `cp312` wheel은 정식 릴리스에도 있습니다). `chromadb==0.5.23`은 이 버전을 범위가 아니라 `chroma-hnswlib==0.7.6`으로 정확히 못박아서(직접 확인: `chroma-hnswlib==0.7.5`를 강제 지정하면 uv가 "unsatisfiable"라며 거부합니다) 다른 버전으로 비켜갈 수도 없습니다. 결국 Python 3.12 이상인 Windows에서 이 두 줄을 설치하려면 Visual Studio C++ 빌드 도구로 소스를 직접 컴파일해야 합니다. 그런데 **이 컴퓨터에는 그 도구가 있습니다** — Build Tools 2022(MSVC 14.44)와 VS Community 2026(MSVC 14.50), 둘 다 C++ 워크로드까지 포함해 이 시리즈를 시작하기 몇 달 전부터 설치되어 있었습니다(직접 확인). 위 메시지는 도구가 없다는 뜻이 아니라, setuptools가 있는 도구를 읽어 들이지 못했다는 뜻입니다. setuptools는 `vcvarsall.bat`을 찾아 실행한 뒤 그 출력에서 `INCLUDE`·`LIB` 같은 값을 거두어 갑니다. 여기서는 `vcvarsall.bat`을 제대로 찾아냈는데(가장 최신인 VS 2026 쪽), 그 배치 파일이 `vswhere.exe`를 찾지 못했다는 줄을 표준오류로 뱉고, setuptools가 표준오류를 표준출력에 합친 뒤 통째로 UTF-16으로 해석하는 바람에 전체가 깨져 **변수가 한 개도 파싱되지 않습니다**(직접 확인 — 두 스트림을 분리해서 실행하면 같은 배치 파일이 149줄짜리 환경을 정상적으로 내놓습니다). 빈 환경을 받은 setuptools는 그것을 "호환되는 Visual Studio 설치가 없다"고 보고합니다. 독자에게 필요한 답은 어느 쪽이든 같습니다 — 컴파일러를 새로 깔아도 이 벽은 그대로이므로, 사전 빌드 wheel이 있는 Python 3.11로 가상환경을 다시 만드는 편이 빠릅니다.
