...
- Produces (`scaffold.mjs`): `nextDayLine(nextDay, nextExists): string`, `readmeSkeleton(day, nextDay | undefined, nextExists?): string`, `linkPreviousDay(prevDay, nextDay, root): boolean`, `scaffoldDay(dayNumber, { days?, root? }): string` (만든 폴더 경로). CLI `node scaffold.mjs 001`.
