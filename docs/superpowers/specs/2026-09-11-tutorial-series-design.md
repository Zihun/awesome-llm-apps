...
14. `uv run` 명령에는 모두 `--no-project`가 붙어 있다. 이 저장소는 루트에 `pyproject.toml`과 `uv.lock`이 있어 uv가 루트를 프로젝트로 인식하므로, 이 플래그 없이 `uv run`을 쓰면 독자가 앱 폴더에 만든 가상환경이 아니라 루트 환경이 쓰인다(2026-09-21 Day 1~17 전체에서 발견 — Day 18만 처음부터 이 플래그를 붙이고 있었다).
