...
- 분량 기준: 본문 1,000~1,500단어(한국어) + 코드 발췌. 60~90분 안에 끝나도록 한다. 새로 가르치는 내용이 많은 날은 이 범위를 넘어도 되지만, 되풀이 때문에 넘는 것은 안 된다. 자주 나오는 되풀이 두 가지는 (1) 앞선 일차가 이미 가르친 사실을 다시 확인 블록으로 증명하는 것, (2) 앞 스텝에서 밝힌 사실을 뒤 스텝에서 길게 다시 설명하는 것이다. 둘 다 한 문장으로 줄이고 앞을 가리킨다. 이 단어 수는 그 자체가 목표가 아니라 60~90분 예산의 대리 지표이며, 실제로 시간이 맞아떨어지는 날들은 분당 본문 17~22단어 선을 따른다. 정당하게 분량이 많은 날은 이 대역을 넘기는 데서 그치지 않고 머리말의 예상 소요 시간을 그 분량에 맞는 현실적인 값으로 바꾸고 Day 17처럼 짧은 이유절을 덧붙인다.
- **단어 수는 `npm run words`로 센다.** 세는 방법을 정하지 않았더니 일차마다 제각기 세어, 같은 문서를 두고 1,997과 3,057이 함께 보고된 적이 있다. 기준이 되려면 방법이 하나여야 한다 — 코드 펜스와 인라인 코드, 제목·표·인용·이미지·체크리스트를 빼고 남는 산문만 센다. `npm run words -- dayNNN`으로 자기 날을 확인하고, 그 수치로 예상 소요 시간을 정한다.
- 이 대역은 통과·불통과를 가르는 문턱이 아니라 **들여다볼 곳을 알려 주는 표시**다(2026-09-22 기준 27일차 중앙값 17.9, 범위 12.0~24.2). 벗어났다면 분량이 부풀었거나 시간이 부풀었거나 둘 중 하나이니 어느 쪽인지 보고 고친다. 손으로 돌려 보는 시간이 읽는 시간보다 큰 날은 대역 아래로 내려가는 것이 정상일 수 있고, 그때는 왜 그런지 이유절에 적는다.
- **대역에 맞추려고 내용을 깎지 않는다.** 무엇을 쓸지는 사실과 필요가 정하고, 시간은 거기에 따라온다. 비율이 어긋나면 고칠 것은 대개 머리말의 시간이지 본문이 아니다 — Day 028은 1,040낱말/40분(26.0)에서 산문을 줄여 850낱말로 맞췄는데, 시간을 50분으로 올렸으면 아무것도 잃지 않고 20.8이 됐다. 되풀이를 덜어 내는 것은 언제나 옳지만, 대역이 목표가 되는 순간 이 지표는 품질을 지키는 대신 해치기 시작한다.
...
**테마 `_tools/theme.d2`.** 모든 `.d2`는 첫 줄에서 테마를 가져온다. 종류를 나타내는 기본 클래스 다섯 개와 각각의 `-new`, `-todo` 변형 15개, 그리고 묶음 전용 `group` 하나로 모두 16개다.

| 클래스 | 용도 | 표현 |
|---|---|---|
| `person` | 사용자 | `shape: person` |
| `ours` | 이 리포의 코드(앱, 에이전트, 도구 함수) | 흰 사각형, 잉크색 테두리 |
| `ext` | 외부 API, LLM 제공자, 그리고 이 PC에서 제3자 런타임(Ollama·llama.cpp 등)이 돌리는 모델 | `shape: cloud`, 아주 옅은 회색 |
| `store` | DB, 벡터 저장소, 캐시 | `shape: cylinder` |
| `file` | 파일, 문서, 설정 | `shape: page` |
| `<기본>-new` (예: `ours-new`) | 이번 스텝에서 추가된 요소 | 주황 테두리 + 옅은 주황 채움 |
| `<기본>-todo` (예: `ext-todo`) | 아직 만들지 않았거나 이번 스텝 실행에서 거치지 않는 요소 | 흐린 회색 점선 |
| `group` | 노드가 아니라 "이것들은 한 덩어리"라는 경계 | 옅은 바탕, 머리카락 테두리, 흐린 라벨. 종류를 주장하지 않는다. 변형이 없으므로 step에서 흐려지지 않고 처음부터 보인다 — 묶음은 만들어 가는 대상이 아니라 읽기 위한 표시다 |

로컬 모델도 `ext`다 — 이 리포의 코드가 아니기 때문이다. 이 PC에서 돈다는 것은 라벨로 밝힌다(예: `Llama 3.2 (Ollama, 로컬)`). 2026-09-23 이전의 날들은 `ours`·`store`가 섞여 있었고 재배치 때 맞춘다.

묶음 컨테이너에 `ours`를 주면 안 된다 — `외부 API`를 묶은 상자가 "이 리포의 코드"로 칠해지면 겉모습이 거짓말을 한다(2026-09-21에 9개 일차 12곳을 `group`으로 고쳤다). `group`은 step이 덮어쓰지 않으므로 `-new`/`-todo` 변형을 두지 않는다.
...
- **색이 아니라 모양이 종류를 말한다.** 강조색은 주황 하나뿐이고 `-new`에만 쓴다. 두 번째 강조색을 들이지 않는다 — 두 개가 되는 순간 "이번에 새로 붙은 것"이라는 신호가 사라진다.
- **글자는 12px 하나로 간다.** 한글은 그 아래에서 뭉개진다. 라벨이 길어 박스에 안 맞으면 글자를 줄이지 말고 라벨을 줄인다. 연결선 라벨은 `**` 글롭이 닿지 않아 테마가 따로 지정한다.
- 한 그림의 노드는 12개 이하. 넘치면 컨테이너로 묶는다.
- **중첩 컨테이너의 `direction`은 엣지가 하나라도 있으면 무시되고, 루트 방향이 그림 전체를 지배한다**(2026-09-21 확인: 루트 down + 안쪽 right = 159×296 세로, 루트 right + 안쪽 down = 288×163 가로. `dagre`도 같다). 그래서 중첩으로는 가로·세로를 섞을 수 없고, 어떻게 손봐도 "전부 길쭉" 아니면 "전부 납작"으로 갈린다. **2차원 배치가 필요하면 `grid-rows` / `grid-columns`를 쓴다** — 루트에도 컨테이너에도 먹고 엔진과 무관하다. 다만 grid 안의 화살표는 꺾이지 않으므로, grid는 안쪽에 화살표가 없는 묶음에만 쓴다(아래 화살표 규칙).
- **그리드 컨테이너는 자기 자손과 엣지를 가질 수 없다.** `box`에 `grid-columns`를 주고 `box -> box.a`를 쓰면 D2가 `edge from grid diagram "box" cannot enter itself`로 거부한다(2026-09-21 확인). 바깥에서 컨테이너로 들어오는 엣지는 괜찮고, 문제가 되는 것은 컨테이너 자신이 한쪽 끝인 경우뿐이다. 그럴 때는 엣지의 출발점을 컨테이너의 자식으로 내리거나, 두 엣지를 하나로 합친다.
- **연결선은 남의 도형을 가로지르면 안 된다**(`check.mjs`가 검사한다). 그리드는 엣지를 보지 않고 자리를 정하고, D2는 그 위에 **꺾임 없는 직선**을 긋는다 — 그래서 멀리 떨어진 두 칸을 이으면 사이에 있는 상자를 그대로 관통한다. ELK 배치는 돌아가게 경로를 잡지만 같은 내용이 두 배 가까이 길어진다(2026-09-23 측정: Day 005 overview가 943×662 → 1083×1479) — 다만 이 부담은 세로 상한을 1000px로 올리며 약해졌다(아래). **그 층을 ELK에 맡겨 꺾어 돌아가게 하는 것이 기본이고, 묶음끼리 잇는 것(엣지를 컨테이너 수준으로 올리는 것)도 된다** — 자식 하나하나를 가리키는 대신 묶음끼리 이으면, Day 005는 1137×440이 되면서 교차가 6건에서 0건으로 사라진 적도 있다.
- **종횡비를 지키는 아이콘(사람·구름·원통·문서)을 그리드 칸에 혼자 두지 않는다.** 그리드가 칸 크기를 줄맞춤하면 그 도형이 칸을 채우려고 통째로 늘어난다 — Day 031의 사람 아이콘이 **761×761**까지 커졌다. 테마에서 `width`/`height`를 줘도 막히지 않는다(2026-09-23 확인: 그리드 늘리기가 이긴다). 고치는 법은 구조뿐이다 — Day 019처럼 형제와 함께 컨테이너에 넣으면 같은 그림에서 64×63이 된다. `check.mjs`가 아이콘 높이 200px 상한으로 검사한다. **사람은 폭도 200px 상한으로 본다** — Day 045의 사람이 643×64로 옆으로 번졌다. 구름·원통·문서는 라벨이 길어 옆으로 넓어지는 것이 정상이라 폭을 재지 않는다.
- **화살표는 서로 겹치지 않고, 글자를 지나거나 글자에 걸치지 않는다 — 곧게 그어서 그렇게 안 되면 꺾는다**(2026-09-23 사용자 지시, `check.mjs` 20~23). 네 가지: (1) 두 화살표가 같은 선을 나눠 쓰지 않는다. (2) 선은 다른 화살표의 라벨이나 묶음 제목을 지나지 않는다 — sequence 그림에서 수명선이 메시지 라벨을 관통하는 것도 같다. (3) 라벨은 도형이나 다른 라벨 위에 얹히지 않는다 — 화살표가 라벨보다 짧으면 라벨이 양쪽 상자를 덮고 선은 보이지 않게 된다. (4) 곧은 화살표는 수평·수직 한 줄일 때만 쓰고, 나머지는 **꺾인 화살표**로 긋는다. 비스듬한 선은 쓰지 않는다.
- **꺾인 화살표는 ELK가 자리를 정한 층에서만 나온다.** grid로 자리를 정하면 D2는 칸 중심끼리 직선을 그을 뿐 경로를 잡지 않는다(2026-09-23 확인: 한 줄 세 칸에서 1번→3번 화살표가 2번 상자를 관통하고 1번→2번 화살표와 같은 선에 겹쳤다). 그래서 **화살표가 오가는 층에는 `grid-rows`/`grid-columns`를 주지 않고 ELK에 맡긴다.** grid는 안쪽에서 화살표가 오가지 않는 묶음(예: 외부 서비스 셋을 한 상자에 모아 두기)에만 쓰고, 그런 묶음에는 바깥 화살표를 묶음 자체에 잇는다. 측정(2026-09-23): overview 57장의 화살표 382개 중 꺾인 것 3개, 비스듬한 선 241개, 53장이 위 규칙 위반. 같은 overview 5장을 ELK로만 배치하자 위반 0.
- **겹침이나 관통을 피하려고 화살표를 지우지 않는다 — 꺾어서 살린다.** overview에 둘지 말지는 구조를 그리느냐 순서를 그리느냐로 정하지, 선을 긋기 어렵다는 이유로 정하지 않는다.
- **렌더된 `.svg`의 높이는 1000px 이하여야 한다**(`sequence.*`는 메시지 하나가 행 하나라 1500px 그대로). `check.mjs`가 검사한다. 꺾인 화살표를 쓰려면 그 층을 ELK에 맡겨야 하고, ELK는 층을 쌓아 그림을 세로로 키운다(실측 626~1396px) — 2026-09-23 사용자가 700 대신 1000px을 골랐다. 넘치면 안쪽에 화살표가 없는 묶음만 grid로 좁히고, 노드와 라벨을 줄인다.
- **세로 상한의 예외는 사용자 결정으로만 둔다.** `_tools/height-exceptions.json`에 적힌 날은 그 날의 overview·stepN 그림에 한해 적힌 높이까지 허용한다(그보다 커지면 다시 걸린다). 2026-09-26에 사용자가 7일(006·010·016·023·025·027·034)을 허락했다 — 모두 화살표 규칙은 통과하고, 실제 관계를 지우거나 합치지 않고는 더 줄일 수 없었던 날이다. 예외에는 이유를 함께 적는다.
- **`overview.d2`는 구조를 그리지 순서를 그리지 않는다.** 요청이 오가는 순서는 `sequence.d2`가 이미 맡고 있다. overview가 `사용자 → UI → 에이전트 → API → 도구 → …`처럼 한 줄짜리 체인이 되면 ELK가 홉마다 층을 하나씩 쌓아 세로로 길어진다 — 같은 내용을 관련된 것끼리 묶어 그리면 훨씬 납작해진다(2026-09-21 확인: Day 019 overview를 체인에서 묶음으로 바꾸자 768×1384 → 1093×446).
- 렌더된 `.svg`의 너비는 **1200px 이하**여야 한다(`sequence.*`는 배우가 가로로 늘어서므로 1400px). 본문이 그림을 축소하면 12px 글자가 같이 작아지고 한글이 먼저 뭉개진다. 라벨이 긴 상자를 `grid-columns`로 넷씩 세우면 폭이 금세 넘친다 — 줄을 나눈다(2026-09-21 확인: Day 021의 `grid-columns: 4`를 2로 바꾸자 1319×566 → 758×669). `check.mjs`가 검사한다.
- **한글 글리프는 SVG 안에 들어간다.** D2가 기본으로 쓰는 Source Sans Pro에는 한글이 없어서, 그대로 두면 (1) 글자 폭을 라틴 기준으로 재어 박스가 어긋나고 (2) 한국어 폰트가 없는 독자에게 두부(□)로 보인다. `_tools/fonts/build.py`가 Noto Sans KR(OFL)에서 `.d2`가 실제로 쓰는 글자만 추려 굽고, 렌더러가 그것을 D2에 넘긴다. 새 일차가 새 글자를 쓰면 `check.mjs`가 잡으므로 `python fonts/build.py` 후 `npm run render -- --force`를 돌린다. 서브셋은 `.d2`뿐 아니라 각 일차 README의 한국어 산문에서도 글자를 모은다 — 다이어그램 라벨만 긁으면 글꼴이 글을 제약하게 되고, 실제로 그 때문에 한 번 단어가 바뀌었다.

**렌더 결과.** `.svg`는 같은 폴더에 같은 이름으로 저장하고 커밋한다. README는 `![설명](diagrams/이름.svg)`로 참조한다.

## 6. 도구 파이프라인

**여러 일차를 동시에 쓸 때의 커밋 규칙.** git 인덱스는 저장소에 하나뿐이라 동시에 도는 작성자들이 스테이징 영역을 공유한다. `git add <내 폴더>` 뒤에 맨몸 `git commit`을 하면 **남이 스테이징해 둔 파일까지 함께 커밋된다**. 그래서 커밋은 반드시 경로를 지정해 한다 — `git commit -F - -- docs/tutorials/dayNNN-…` 형태면 인덱스에 무엇이 올라와 있든 그 경로만 들어간다.
- **경로는 "내가 고친 폴더"만 적는다.** `-- docs/tutorials` 같은 넓은 경로는 경로를 지정한 것처럼 보이지만 그 아래 남의 작업 중인 파일까지 담는다(2026-09-23에 내가 그렇게 해서 한 에이전트의 편집 중 파일을 쓸어 담았다).
- **미추적 파일은 `git add`를 먼저 해야 한다.** `git commit -- <폴더>`는 이미 추적 중인 파일의 변경만 담고 **새로 만든 파일은 빠뜨린다** — 새 일차의 `diagrams/`가 통째로 누락되는 형태로 나타난다. 커밋 뒤에 `git status --short -- <내 폴더>`로 남은 것이 없는지 확인한다.
- **`git reset`은 절대 쓰지 않는다.** 2026-09-22에 한 작성자가 자기 커밋이 섞인 것을 `git reset --soft HEAD~1`로 되돌렸는데, 그 사이 HEAD는 이미 다른 작성자의 커밋이었다 — 남의 커밋을 고아로 만들었다. 내용 손실은 없었지만(그 작성자가 재개되며 다시 커밋했다) 되돌리기는 동시 작업에서 남의 이력을 건드리는 일이 된다. 섞였으면 되돌리지 말고 보고한다.
- 커밋 전후로 `git status --short -- <내 폴더>`만 본다. 저장소 전체 status는 남의 작업 중인 파일을 보여 주므로 판단 근거가 되지 못한다.
...
12. 더 해보기의 줄 번호 인용은 백틱 안에 파일 경로까지 들어 있다(서술문 속 "29행의 …"는 제외).
13. 코드 위치 인용이 저장소 밖(`.venv/`, `site-packages/`)을 가리키지 않는다. 서드파티 패키지 내부는 "소스로 확인"으로 적고 패키지와 버전을 밝힌다(2026-09-21 Day 2·12·13 병합 후 발견 — 다른 머신의 `.venv` 경로가 인용에 그대로 남아 있었다).
14. `uv run` 명령에는 모두 `--no-project`가 붙어 있다. 이 저장소는 루트에 `pyproject.toml`과 `uv.lock`이 있어 uv가 루트를 프로젝트로 인식하므로, 이 플래그 없이 `uv run`을 쓰면 독자가 앱 폴더에 만든 가상환경이 아니라 루트 환경이 쓰인다(2026-09-21 Day 1~17 전체에서 발견 — Day 18만 처음부터 이 플래그를 붙이고 있었다). 검사는 셸 펜스(bash, sh, powershell) 안쪽 명령만 본다. 태그 없는 펜스는 실패를 보여주는 시연일 수 있으므로 그곳의 미표시 `uv run`은 문서가 독자를 실수하게 만들지 않는 이상 보고되지 않는다.
15. 다이어그램에 쓰인 글자가 모두 임베드 폰트(`fonts/coverage.txt`)에 있다.
16. 렌더된 `.svg`의 높이가 1000px 이하다(sequence는 1500px). `_tools/height-exceptions.json`에 적힌 날의 overview·step은 거기 적힌 높이까지.
17. 연결선이 자기 양 끝이 아닌 잎 도형을 가로지르지 않는다.
18. 종횡비를 지키는 아이콘이 그리드 칸에 맞춰 늘어나지 않았다(높이 200px 이하, 사람은 폭도 200px 이하).
19. 단계 공개 불변식 넷을 지킨다 — stepN의 노드 id 집합이 overview와 같고(더 있거나 빠지면 오류), 클래스가 `-todo`로 끝나는 노드 수가 이전 step보다 늘지 않으며, 마지막 step에는 `-todo` 노드가 없고, `-todo`였던 노드가 다음 step에서 `-todo`를 벗어나면 그 step에서 `-new`로 끝난다(처음 드러나는 단계는 주황으로 — 평상 → `-new` 재강조는 관례이므로 막지 않는다).
20. 서로 다른 두 연결선이 같은 직선 위에서 겹치지 않는다(§5 화살표 규칙).
21. 선이 자기 라벨이 아닌 다른 화살표의 라벨이나 묶음 제목을 지나지 않는다(§5).
22. 줄인 라벨 상자가 잎 도형이나 다른 라벨과 겹치지 않는다(§5).
23. `sequence`가 아닌 그림에서, 곧은 화살표는 수평·수직 한 줄일 때만 쓰고 비스듬한 화살표는 없다(§5).
