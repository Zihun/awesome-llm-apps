...
- **연결선은 남의 도형을 가로지르면 안 된다**(`check.mjs`가 검사한다). 그리드는 엣지를 보지 않고 자리를 정하고, D2는 그 위에 **꺾임 없는 직선**을 긋는다 — 그래서 멀리 떨어진 두 칸을 이으면 사이에 있는 상자를 그대로 관통한다. ELK 배치는 돌아가게 경로를 잡지만 같은 내용이 두 배 이상 길어진다(2026-09-23 측정: Day 005 overview가 943×662 → 1083×1479). **통하는 해법은 셋째다 — 엣지를 컨테이너 수준으로 올린다.** 자식 하나하나를 가리키는 대신 묶음끼리 잇는다. 같은 그림이 1137×440이 되면서 교차가 6건에서 0건으로 사라졌다. 개요도는 어차피 구조를 그리는 자리이므로 이 편이 내용에도 맞는다.
- **종횡비를 지키는 아이콘(사람·구름·원통·문서)을 그리드 칸에 혼자 두지 않는다.** 그리드가 칸 크기를 줄맞춤하면 그 도형이 칸을 채우려고 통째로 늘어난다 — Day 031의 사람 아이콘이 **761×761**까지 커졌다. 테마에서 `width`/`height`를 줘도 막히지 않는다(2026-09-23 확인: 그리드 늘리기가 이긴다). 고치는 법은 구조뿐이다 — Day 019처럼 형제와 함께 컨테이너에 넣으면 같은 그림에서 64×63이 된다. `check.mjs`가 아이콘 높이 200px 상한으로 검사하며, **폭은 재지 않는다**: 라벨이 길어 구름이 옆으로 넓어지는 것은 정상이고 늘어남은 높이에만 나타난다.
- **렌더된 `.svg`의 높이는 700px 이하여야 한다**(`sequence.*`는 메시지 하나가 행 하나라 1500px). `check.mjs`가 검사한다. 한 페이지에 그림이 6~8장 들어가므로, 장당 700px을 넘기면 본문이 그림 벽이 된다.
