...
- **한글 글리프는 SVG 안에 들어간다.** D2가 기본으로 쓰는 Source Sans Pro에는 한글이 없어서, 그대로 두면 (1) 글자 폭을 라틴 기준으로 재어 박스가 어긋나고 (2) 한국어 폰트가 없는 독자에게 두부(□)로 보인다. `_tools/fonts/build.py`가 Noto Sans KR(OFL)에서 `.d2`가 실제로 쓰는 글자만 추려 굽고, 렌더러가 그것을 D2에 넘긴다. 새 일차가 새 글자를 쓰면 `check.mjs`가 잡으므로 `python fonts/build.py` 후 `npm run render -- --force`를 돌린다.
