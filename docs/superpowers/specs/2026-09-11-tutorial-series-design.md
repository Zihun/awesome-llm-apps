...
| `ext` | 외부 API, LLM 제공자 | `shape: cloud`, 아주 옅은 회색 |
...
19. 단계 공개 불변식 셋을 지킨다 — stepN의 노드 id 집합이 overview와 같고(더 있거나 빠지면 오류), 클래스가 `-todo`로 끝나는 노드 수가 이전 step보다 늘지 않으며, 마지막 step에는 `-todo` 노드가 없다.
