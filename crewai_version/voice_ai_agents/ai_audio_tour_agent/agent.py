...
def create_architecture_agent(llm):
    """Create Architecture Agent using CrewAI"""
    search_tool = SerperDevTool()

    return Agent(
        role="ArchitectureAgent",
        goal=ARCHITECTURE_AGENT_INSTRUCTIONS,
        backstory="Expert in architectural analysis and urban design",
        llm=llm,
        tools=[search_tool],
        verbose=True,
...
def create_culinary_agent(llm):
    """Create Culinary Agent using CrewAI"""
    search_tool = SerperDevTool()

    return Agent(
        role="CulinaryAgent",
        goal=CULINARY_AGENT_INSTRUCTIONS,
        backstory="Expert in culinary traditions and local cuisine",
        llm=llm,
        tools=[search_tool],
        verbose=True,
        allow_delegation=False
...
def create_culture_agent(llm):
    """Create Culture Agent using CrewAI"""
    search_tool = SerperDevTool()

    return Agent(
        role="CulturalAgent",
        goal=CULTURE_AGENT_INSTRUCTIONS,
        backstory="Expert in cultural anthropology and local traditions",
        llm=llm,
        tools=[search_tool],
        verbose=True,
        allow_delegation=False
...
def create_historical_agent(llm):
    """Create Historical Agent using CrewAI"""
    search_tool = SerperDevTool()

    return Agent(
        role="HistoricalAgent",
        goal=HISTORY_AGENT_INSTRUCTIONS,
        backstory="Expert historian specializing in local and regional history",
        llm=llm,
        tools=[search_tool],
        verbose=True,
        allow_delegation=False
...
def create_orchestrator_agent(llm):
    """Create Orchestrator Agent using CrewAI"""
    return Agent(
        role="OrchestratorAgent",
        goal=ORCHESTRATOR_INSTRUCTIONS,
        backstory="Expert tour guide coordinator who assembles comprehensive tour experiences",
        llm=llm,
        verbose=True,
        allow_delegation=False
    )
...
def create_planner_agent(llm):
    """Create Planner Agent using CrewAI"""
    return Agent(
        role="PlannerAgent",
        goal=PLANNER_INSTRUCTIONS,
        backstory="Expert tour planning specialist who optimizes time allocation",
        llm=llm,
        verbose=True,
        allow_delegation=False
    )
