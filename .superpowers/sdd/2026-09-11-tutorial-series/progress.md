...
**멈춤 지점 (2026-09-23, 19d26a4로 푸시함)**: RAG 볼륨(Days 47~70) 진행 중.
**Days 47~52 작성·검증·커밋·푸시 완료**(목차 진도 52/164). Days 53·54·55·56은 에이전트가 쓰는 중 —
Day 53은 README가 디스크에 다 써졌지만 아직 커밋 전이라 목차에서 일부러 ⬜로 두었다. 이 넷이 들어오면
`npm run roadmap`을 다시 돌려 목차를 맞출 것. 그 다음은 Day 57부터 넷씩.

**배치가 끝나면 바로 할 일 (검사 19번)**: 점진적 공개 불변식을 `check.mjs`에 넣는다. 코드는 이미 쓰고
테스트까지 통과시켜 뒀다 — `lib/reveal.mjs`와 `test/reveal.test.mjs`(5개, 총 40개가 된다)로 들어가고,
초안은 이 세션의 스크래치패드에 있다. **지금 넣지 않는 이유는 하나뿐이다**: 돌고 있는 브리프들이
`npm test` 35개를 기대하고 있어서다. 초안이 사라졌으면 다시 쓰면 된다 — 불변식 셋은 아래에 적어 뒀다.
